                               ...
                               gkick_real density)
{
	if (kick == NULL) {
        ...
        enum geonkick_error res;
        res = gkick_synth_set_osc_noise_density(kick->synths[kick->per_index],
                                                osc_index,
                                                density);
        if (res == GEONKICK_OK && kick->synths[kick->per_index]->buffer_update)
                geonkick_wakeup(kick);
...
enum geonkick_error
geonkick_distortion_enable(struct geonkick *kick, bool enable)
{
        if (kick == NULL) {
                gkick_log_error("wrong arguments");
                return GEONKICK_ERROR;
                             ...
                             enum gkick_distortion_type type)
{
        ...
        enum geonkick_error res;
        res = gkick_synth_distortion_set_type(kick->synths[kick->per_index],
                                              type);
        if (res == GEONKICK_OK && kick->synths[kick->per_index]->buffer_update)
                geonkick_wakeup(kick);
        return res;
                             ...
                             enum gkick_distortion_type *type)
{
        ...
        return gkick_synth_distortion_get_type(kick->synths[kick->per_index],
                                               type);
}
                               ...
                               bool enable)
{
        ...
        if (!osc)
                return GEONKICK_ERROR;
        enum geonkick_error res;
        res = gkick_distortion_enable(osc->distortion, enable);
        if (res == GEONKICK_OK) {
                                 ...
                                 enum gkick_distortion_type *type)
{
        ...
        if (!osc)
                return GEONKICK_ERROR;
        return gkick_distortion_get_type(osc->distortion, type);
}
                                       ...
                                       gkick_real value)
{
        ...
        if (!osc)
                return GEONKICK_ERROR;
        enum geonkick_error res;
        res = gkick_distortion_set_in_limiter(osc->distortion, value);
        if (res != GEONKICK_OK)
                return res;
        bool enabled = false;
        gkick_distortion_is_enabled(osc->distortion, &enabled);
                                        ...
                                        gkick_real value)
{
        ...
        if (!osc)
                return GEONKICK_ERROR;
        enum geonkick_error res;
        res = gkick_distortion_set_out_limiter(osc->distortion, value);
        if (res != GEONKICK_OK)
                return res;
        bool enabled = false;
        gkick_distortion_is_enabled(osc->distortion, &enabled);
                                  ...
                                  gkick_real drive)
{
        ...
        if (!osc)
                return GEONKICK_ERROR;
        enum geonkick_error res;
        res = gkick_distortion_set_drive(osc->distortion, drive);
        if (res != GEONKICK_OK)
                return res;
        bool enabled = false;
        gkick_distortion_is_enabled(osc->distortion, &enabled);
                                  ...
                                  gkick_real *drive)
{
        ...
        if (!osc)
                return GEONKICK_ERROR;
        return gkick_distortion_get_drive(osc->distortion, drive);
}
